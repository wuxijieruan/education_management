<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" style="margin-bottom:10px;">
      ...
      <el-breadcrumb-item>轮播栏编辑</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-form :inline="true" :model="form" class="user-search" ref="form">
      <el-form-item>
        ...
        <el-button size="small" type="primary" @click="submitEdit">保存</el-button>
      </el-form-item>
    </el-form>
...
import { gradeSubjectGet, bannerPut, courseGet } from "@/api/getData";
...
export default {
  ...
  created() {
    ...
    this.form = data.row;
    // 图片
    if (data.row.bannerPicUrl) {
      this.form.bannerPicUrl = data.row.bannerPicUrl.split(",");
      this.form.bannerPicUrl.forEach(element => {
        var fileList = {};
        var name = element.split("/");
        fileList.name = name[name.length - 1];
        fileList.url = element;
  ...
  methods: {
    ...
    async submitEdit() {
      ...
      try {
        ...
        console.log("轮播栏编辑", this.form);

        const res = await bannerPut(this.form);
        if (res.status == 200) {
          this.$message({
            message: "提交成功",
            type: "success"
