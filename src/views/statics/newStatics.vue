    ...
    export2Excel() {
                ...
                if (that.form.resourceType == 0) {//累计
                    require.ensure([], () => {
                        ...
                        const filterVal = ['useDate','newUserQtyTotalDay', 'newTouristQtyTotalDay', 'newComVipQtyTotalDay','newEnterpriseVipQtyTotalDay','xieCheng',"zhongYin","qianCheng"]; // 导出的表头名
                        const tHeader = ['日期','总人数','游客', '初级VIP','企业VIP','携程','中银电池','前程照明']; // 导出的表头字段名
                        ...
                        export_json_to_excel(tHeader, data, `${currentdate}用户累计新增excel`);// 导出的表格名称，根据需要自己命名
                      })
                    } else if(that.form.resourceType == 1){//每日
                      require.ensure([], () => {
                        const { export_json_to_excel } = require('../../excel/Export2Excel'); //这里必须使用绝对路径
                         const filterVal = ['useDate','newUserQtyPerDay', 'newTouristQtyPerDay', 'newComVipQtyPerDay','newEnterpriseVipQtyPerDay','xieCheng',"zhongYin","qianCheng"]; // 导出的表头名
                        const tHeader = ['日期','总人数','游客', '初级VIP','企业VIP','携程','中银电池','前程照明']; // 导出的表头字段名
                        const list = that.excelData;
                        const data = that.formatJson(filterVal, list);
                        let datetime=new Date()
                        ...
                        var currentdate = year + seperator1 + month + seperator1 + strDate;
                          ...
                          export_json_to_excel(tHeader, data, `${currentdate}用户每日新增excel`);// 导出的表格名称，根据需要自己命名
