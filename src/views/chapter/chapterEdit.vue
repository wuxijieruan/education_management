<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" style="margin-bottom:10px;">
      ...
      <el-breadcrumb-item>章节编辑</el-breadcrumb-item>
    </el-breadcrumb>
...
import {
  courseChapterPut,
  courseChapterDetail,
  courseChapterFilePut
} from "@/api/getData";
import { videoPlayer } from "vue-video-player";
import "video.js/dist/video-js.css";
import { videoUrl, imgUrl, imagesUrl, newvideoUrl ,zipFileUrl} from "@/config/env";
export default {
  data() {
    return {
  ...
  created() {
    ...
    console.log("章节编辑query", data);
    this.form.courseChapterId = data.chapterId;
  ...
  methods: {
    async getChapter() {
      try {
        const res = await courseChapterDetail(this.chapterId);
        if (res.status == 200) {
          console.log("章节信息详情", res.data);
          // 章节名称和顺序
          var courseChapter = res.data.courseChapter;
          this.form.courseChapterName = courseChapter.courseChapterName;
          this.form.chapterIndex = courseChapter.chapterIndex;
          this.form.lookNo = courseChapter.lookNo;
          this.form.courseId = courseChapter.courseId;
          // 章节书评文件
          ...
          console.log("this.elBookList", this.elBookList);
          // 章节音频
          if (res.data.audioList.length != 0) {
            res.data.audioList.forEach((element, index) => {
              ...
              this.voiceList.push(list);
            });
            console.log("课程音频", this.voiceList);
            this.audioUrl = this.voiceList[0].fileUrl;
          }
          // 章节图片
          if (res.data.imgList.length != 0) {
            var list3 = [];
            res.data.imgList.forEach((element, index) => {
              console.log("element", element);
              var list = {
                chapterId: element.chapterId,
                fileName: element.fileName,
              ...
              // console.log(list);
              this.allfileList.push(list);
              var url = list.fileUrl;
              var name = list.fileName;
              var response = {
                url: element.fileUrl
              };
              list3.push({ url, name, response });
            });
            this.imgList = list3;
            console.log("课程图片", this.imgList);
          }
          // 章节视频
          if (res.data.videoList.length != 0) {
            res.data.videoList.forEach((element, index) => {
              // console.log("element", element);
              if (element.fileUrl.indexOf(".") > 0) {
                var list = {
                  chapterId: element.chapterId,
                  fileName: element.fileName,
                  fileType: 1,
            ...
            console.log("课程视频", this.videoList);
          ...
          console.log("allfileList", this.allfileList);
        } else {
          this.$message({
            type: "error",
            message: res.error
    ...
    submitAdd(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          const res = await courseChapterPut(this.form);
          ...
          if (res.status == 200) {
            ...
            if (this.allfileList.length != 0) {
              this.allfileList.forEach(item => {
                console.log(item);
                item.chapterId = this.form.courseChapterId;
              });
              this.fileAdd();
            } else {
              this.allfileList.push({ chapterId: this.form.courseChapterId });
              this.fileAdd();
            }
          } else {
            this.$message({
    ...
    async fileAdd() {
      try {
        const res = await courseChapterFilePut(this.allfileList);
